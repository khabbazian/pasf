...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
	...
	/*bool mask  = mxGetPr(prhs[1]);*/
	double* inputMask =  mxGetPr( prhs[1] );
	

	unsigned char* mask = calloc(image_size, sizeof(unsigned char));
	...
	for(ii=0; ii<image_size; ++ii)
		if(inputMask[ii] > 0 ){
			...
			mask[ii] = 0; /*zero means no mask*/
			++count;
		}

	if(count < 4)
		mexErrMsgTxt("Too few unmasked input.");
	...
	double* tmp = (double *) calloc(image_size, sizeof(double));
	memcpy(tmp, wrapped, image_size * sizeof(double) );

	unwrap2D(wrapped, unwrapped, mask, rows, cols, false, false); 

	int iter = 0, i=0;
	for(; iter < maxNIter; ++iter){
		/*unwrap2D(tmp, unwrapped, mask, cols, rows, false, false);*/
		unwrap2D(tmp, unwrapped, mask, rows, cols, false, false); 
		if( memcmp(tmp, unwrapped, image_size*sizeof(double)) == 0)
			break;
		memcpy(tmp, unwrapped, image_size * sizeof(double));
	}
	/*if(iter == maxNIter)
		printf("unwrap2D reached maximum number of iterations, %d.", maxNIter);
	*/
